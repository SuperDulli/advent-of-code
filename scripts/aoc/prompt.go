...
func GetPrompt(day, year int, cookie string) {
	...
	// write to file
	directoryName := filepath.Join(util.DirectoryName(), fmt.Sprintf("%d/day%02d", year, day))
	filename := filepath.Join(directoryName, "prompt.md")
	WriteToFile(filename, []byte(prompt))

	fmt.Println("Wrote prompt to file: ", filename)

	// rename directory
	headline := regexp.MustCompile(`--- Day \d+: (.+) ---`).FindStringSubmatch(prompt)
	newDirectoryName := filepath.Join(util.DirectoryName(), fmt.Sprintf("%d/%02d_"+util.ToSnakeCase(headline[1]), year, day))
	err := os.Rename(directoryName, newDirectoryName)
	if err != nil {
		log.Fatalf("renaming directory: %s", err)
	}
	fmt.Println("Renamed directory to: ", newDirectoryName)
